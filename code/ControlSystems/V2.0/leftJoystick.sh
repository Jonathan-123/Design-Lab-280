...
int LEFT = 0;			//Store the value of each joystick, making 
int RIGHT = 0;			//the data persistent across reads
int FAST = 1;
    ...
    while (read_event(js, &event) == 0)
    {
        switch (event.type)
        {
            ...
            case JS_EVENT_AXIS:
                ...
                if (axis < 3)
                   if (axis < 1)
						LEFT = (axes[axis].y) * FAST * -1;
                   if (axis > 1)
						RIGHT = (axes[axis].x) * FAST * -1;
                   printf("%6dL\t%6dR\n", LEFT, RIGHT);
